...
script_folder=/volume1/Folder where your script and .pfx are
# Domain Name for naming file and cert conversion
domain_name=domain.tld
# p12 file
p12_file_path="$script_folder/$domain_name.pfx"
# p12 password
p12cert_password=Password of your certificate
...
if expr "$generate_p12" "=" "true" > /dev/null; then
  ...
  openssl pkcs12 -export -out "$p12_file_path" \
    -in "$letsencrypt_cert_folder/cert.pem" \
    -inkey "$letsencrypt_cert_folder/privkey.pem" \
    -certfile "$letsencrypt_cert_folder/fullchain.pem" \
